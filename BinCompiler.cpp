...
void printVectorOfPairs(const vector<pair<string, string>>& vec) {
    ...
    while (token != "END") {
        ...
        if (token == "KW") {
            if (strcmp(UL, "OPR") == 0) {
                ...
                accept(UL, "INT", token);  // Accept the integer operand
                currentNode->children.push_back(new node("INT", UL));  // Add operand node
            }
...
int checkLabels(noded* head1, noded* head) { // Check undefined labels
    noded* current = head1;
